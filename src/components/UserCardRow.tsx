    ...
    <div className="flex items-center gap-3 px-4 py-3">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          ...
          {user.available ? <span className="badge bg-gold-100 text-gold-700">待機中</span> : null}
        </div>
        <div className="mt-0.5 flex flex-wrap items-center gap-1.5 text-[11px] text-ink-500">
          ...
          {showFee && user.orderFeePerTime ? (
            <span className="text-gold-700">{numberToCredits(user.orderFeePerTime)}/30分</span>
          ) : null}
        </div>
  ...
  const inner = (
    <>
      <div className="relative">
        ...
        />
        {selected ? (
          <span className="absolute inset-0 flex items-center justify-center rounded-lg bg-gold-500/30 text-2xl">✓</span>
        ) : null}
        {user.available ? (
          <span className="absolute left-1 top-1 badge bg-gold-500 text-ink-900">待機中</span>
        ) : null}
      </div>
      ...
      <div className="flex items-center gap-1 text-[10px] text-ink-500">
        {user.birthdayPublished && user.age !== null ? <span>{user.age}歳</span> : null}
        {user.orderFeePerTime ? <span className="text-gold-700">{user.orderFeePerTime}P</span> : null}
      </div>
    </>
  );
