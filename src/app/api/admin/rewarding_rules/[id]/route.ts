...
/** admin settings: GET|PATCH|DELETE /admin/rewarding_rules/:id */
export const GET = route<{ id: string }>(async (_request, { params }) =>
  showResource('rewarding_rules', idSchema.parse(params).id),
);

export const PATCH = route<{ id: string }>(async (request, { params }) =>
  updateResource('rewarding_rules', idSchema.parse(params).id, await jsonBody(request)),
);

export const DELETE = route<{ id: string }>(async (_request, { params }) =>
  destroyResource('rewarding_rules', idSchema.parse(params).id),
);
