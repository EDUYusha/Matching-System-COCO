...
/** admin settings: GET|PATCH|DELETE /admin/roulette_entries/:id */
export const GET = route<{ id: string }>(async (_request, { params }) =>
  showResource('roulette_entries', idSchema.parse(params).id),
);

export const PATCH = route<{ id: string }>(async (request, { params }) =>
  updateResource('roulette_entries', idSchema.parse(params).id, await jsonBody(request)),
);

export const DELETE = route<{ id: string }>(async (_request, { params }) =>
  destroyResource('roulette_entries', idSchema.parse(params).id),
);
