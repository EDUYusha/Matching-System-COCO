...
/** admin settings: GET|PATCH|DELETE /admin/admins/:id */
export const GET = route<{ id: string }>(async (_request, { params }) =>
  showResource('admins', idSchema.parse(params).id),
);

export const PATCH = route<{ id: string }>(async (request, { params }) =>
  updateResource('admins', idSchema.parse(params).id, await jsonBody(request)),
);

export const DELETE = route<{ id: string }>(async (_request, { params }) =>
  destroyResource('admins', idSchema.parse(params).id),
);
