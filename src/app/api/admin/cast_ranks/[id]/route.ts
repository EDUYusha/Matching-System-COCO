...
/** admin settings: GET|PATCH|DELETE /admin/cast_ranks/:id */
export const GET = route<{ id: string }>(async (_request, { params }) =>
  showResource('cast_ranks', idSchema.parse(params).id),
);

export const PATCH = route<{ id: string }>(async (request, { params }) =>
  updateResource('cast_ranks', idSchema.parse(params).id, await jsonBody(request)),
);

export const DELETE = route<{ id: string }>(async (_request, { params }) =>
  destroyResource('cast_ranks', idSchema.parse(params).id),
);
