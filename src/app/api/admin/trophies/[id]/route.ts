...
/** admin settings: GET|PATCH|DELETE /admin/trophies/:id */
export const GET = route<{ id: string }>(async (_request, { params }) =>
  showResource('trophies', idSchema.parse(params).id),
);

export const PATCH = route<{ id: string }>(async (request, { params }) =>
  updateResource('trophies', idSchema.parse(params).id, await jsonBody(request)),
);

export const DELETE = route<{ id: string }>(async (_request, { params }) =>
  destroyResource('trophies', idSchema.parse(params).id),
);
