...
/** admin settings: GET|PATCH|DELETE /admin/company_informations/:id */
export const GET = route<{ id: string }>(async (_request, { params }) =>
  showResource('company_informations', idSchema.parse(params).id),
);

export const PATCH = route<{ id: string }>(async (request, { params }) =>
  updateResource('company_informations', idSchema.parse(params).id, await jsonBody(request)),
);

export const DELETE = route<{ id: string }>(async (_request, { params }) =>
  destroyResource('company_informations', idSchema.parse(params).id),
);
