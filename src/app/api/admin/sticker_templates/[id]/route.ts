...
/** admin settings: GET|PATCH|DELETE /admin/sticker_templates/:id */
export const GET = route<{ id: string }>(async (_request, { params }) =>
  showResource('sticker_templates', idSchema.parse(params).id),
);

export const PATCH = route<{ id: string }>(async (request, { params }) =>
  updateResource('sticker_templates', idSchema.parse(params).id, await jsonBody(request)),
);

export const DELETE = route<{ id: string }>(async (_request, { params }) =>
  destroyResource('sticker_templates', idSchema.parse(params).id),
);
