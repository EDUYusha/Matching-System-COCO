...
/** admin settings: GET|PATCH|DELETE /admin/service_messages/:id */
export const GET = route<{ id: string }>(async (_request, { params }) =>
  showResource('service_messages', idSchema.parse(params).id),
);

export const PATCH = route<{ id: string }>(async (request, { params }) =>
  updateResource('service_messages', idSchema.parse(params).id, await jsonBody(request)),
);

export const DELETE = route<{ id: string }>(async (_request, { params }) =>
  destroyResource('service_messages', idSchema.parse(params).id),
);
