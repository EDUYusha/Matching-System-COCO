...
/** admin settings: GET|PATCH|DELETE /admin/meeting_place_tags/:id */
export const GET = route<{ id: string }>(async (_request, { params }) =>
  showResource('meeting_place_tags', idSchema.parse(params).id),
);

export const PATCH = route<{ id: string }>(async (request, { params }) =>
  updateResource('meeting_place_tags', idSchema.parse(params).id, await jsonBody(request)),
);

export const DELETE = route<{ id: string }>(async (_request, { params }) =>
  destroyResource('meeting_place_tags', idSchema.parse(params).id),
);
