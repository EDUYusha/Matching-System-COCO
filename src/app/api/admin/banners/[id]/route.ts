...
/** admin settings: GET|PATCH|DELETE /admin/banners/:id */
export const GET = route<{ id: string }>(async (_request, { params }) =>
  showResource('banners', idSchema.parse(params).id),
);

export const PATCH = route<{ id: string }>(async (request, { params }) =>
  updateResource('banners', idSchema.parse(params).id, await jsonBody(request)),
);

export const DELETE = route<{ id: string }>(async (_request, { params }) =>
  destroyResource('banners', idSchema.parse(params).id),
);
